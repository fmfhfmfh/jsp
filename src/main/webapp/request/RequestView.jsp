...
<body>
	<%-- 파라미터 : client 서버로 요청을 보낼때 추가적으로 보낸 값
		...
		String getParameter(String param) : param에 해당하는 파라미터 값을 조회한다 
		                                    파라미터맵에서 첫번째 값을 반환            
		String[] getParameterValues(String param) : param에 해당하는 모든 파라미터값을 반환한다    
	 ...
	 --%>
	 <form action="${pageContext.request.contextPath}/request/RequestResponse.jsp" id="change" method="GET">
	 ...
	 GET POST 두가지를 선택 할 수 있는 라디오 버튼을 만들어서
	 FORM 전송시 사용자가 GET,POST 방식을 지정 할 수 있도록<br>
	 <input type="radio" name="request" value="GET">GET<br>
	 <input type="radio" name="request" value="POST">POST
</body>
</html>