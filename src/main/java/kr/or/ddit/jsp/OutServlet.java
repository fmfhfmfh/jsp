...
public class OutServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// ranger 정보를 화면에 표현해줄 out.jsp 파일로 응답 생성을 위힘
		// ranger 정보가 어디인가 담겨야함
		// servlet 에서는 3가지 scope가 사용하능
		// request < session < application
		// 해당요청을 일회성으로만 처리해주면 ranger 이름정보를 보관할 필요가 없고
		// 다른 요청과 관련이 없으므로 request가 적당하다
		
		// servlet 상에서 작성할 코드
		// request 객체에 rangers라는 속성이름으로 ranger정보를 저장
		
		// out.jsp(아직 생성되지 않음 webapp/jsp/out.jsp에 생성)로 요청위임(forward)
		
		
		// out.jsp에서는 ranger라는 속성을 꺼내서 loop를 돌며 화면에 출력
		// ***out 객체를 이용
